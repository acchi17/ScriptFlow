...
## 7. 将来の検討事項: レシピ単位の子プロセス分離

本機能(タブの作成・削除・Load/Save)のスコープには含めないが、将来複数レシピを並列実行可能にする場合に備えて論点を記録する。

**現状の共有アーキテクチャ**: `shared/ScriptRunnerHost.js`(JS)と`shared/PythonRunnerHost.js`(Python)は、それぞれアプリ全体でただ1つの子プロセスを`_ensureProcess()`で遅延生成し、`electron/main.js`/`server/index.js`のモジュールレベル変数`runnerHost`として全レシピ・全ブロック実行で共有している。リクエストcorrelation(`_pending` Map)もソケット管理(`shared/script-runner.js`内の`sockets` Map、キーは`entryId`)も、レシピ/タブという次元を一切持たない。entryIdがUUIDであるため今は衝突していないだけで、構造的な分離ではない。

**分離すべき理由**:
- 子プロセスはシングルスレッドで動作するため、1レシピの重い/ブロックするスクリプトが他の全レシピの実行を止める。
- 子プロセスのクラッシュ/exit時に`_pending`の全リクエストが一括rejectされるため、1レシピの異常が他レシピの実行中処理を巻き込む。
- タブ削除時にプロセスを終了させたくても、今は「アプリ全体で1つ」しかないため終了の影響範囲を1タブに絞れない。

**分離する場合の設計方針(概略)**:
- `runnerHost`をアプリ全体の単一シングルトンではなく、rootId(タブID)をキーにしたMapで保持する形に変える。
- 既存の遅延生成パターン(`_ensureProcess()`)を活かし、タブを開いた時点ではプロセスを起動せず、そのタブで最初にスクリプトが実行された時点で該当タブ用のプロセスを生成する(アイドルタブのコストを避ける)。
- タブ削除時に、対応する`runnerHost`インスタンスの`shutdown()`を呼ぶ(既存の`app.on('before-quit')`と同様のパターン)。
- IPCハンドラ(`script:execute`等)・Web版のAPIルートにrootId/tabIdを渡す必要があり、プロトコル変更を伴う。

**インスタンス設計: `ScriptRunnerHost`/`PythonRunnerHost`をタブごとに複数生成する**。検討した代替案は「単一インスタンスが内部で複数プロセスを管理する(クラス内に`Map<rootId, {process, pending, counter}>`を持つ)」形だったが、採用しない。理由は責務の分離で、代替案では「1プロセス分のIPCプロトコル・リクエスト対応管理」と「複数プロセスのプール管理」という2つの責務が1クラスに混在してしまう。一方、タブごとにインスタンスを生成する案では`ScriptRunnerHost`/`PythonRunnerHost`自体は今の実装(1インスタンス=1プロセス)のまま無改修で再利用でき、「どのタブにどのインスタンスを使うか」というルーティングだけを外側の薄いレジストリ(`Map<rootId, ScriptRunnerHost>`)に切り出せる。これは`electron/main.js`の`ensureRunnerHost()`が既に持つ「遅延生成して返すファクトリ関数」パターンをキー付きMapに変えるだけで実現でき、変更量も最小になる。

**注意点**: 素のPythonインタプリタ起動自体はNode(Electronの`utilityProcess`)と大差ない(実測: 素のPython起動 約60ms、`node -e ""` 約64ms)。ただし`appdata/script_runner.py`が起動時に読み込む標準ライブラリ(特に`asyncio`)のimportで実質約180msかかり、加えてユーザースクリプトが`numpy`/`pandas`等の重いライブラリをimportする場合はプロセスごとにそのコストを個別に払うことになる。遅延生成(`_ensureProcess()`)により各タブでこのコストは「そのタブで最初にスクリプトを実行した時」に一度だけ発生するため、タブ上限(10個)分が同時に重なることはないが、ユーザースクリプトの内容次第でタブ切替直後の初回実行が重くなり得る点は要検討事項として残る。

セクション4の既存方針(実行ロック・ログのタブ単位分離もスコープ外)と一貫させ、この項目も将来必要になった場合に別途設計する。
