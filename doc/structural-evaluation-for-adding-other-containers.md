...
2. **Make `_executeContainer`'s execution strategy swappable per container kind** — It's currently a hardcoded, unconditional `for...of` loop. Bolting branch/loop conditions onto it with `if`/`else` will bloat `_executeContainer`. `ContainerExecutionFactory` (`entry_execution`) already follows this pattern — a "per-container-kind execution strategy" split into small classes and assembled via a factory. This strategy is still chosen **at runtime** — it has nothing to do with static compilation.
